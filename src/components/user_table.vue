                ...
                <div class="col-12">
                    <div class="bg-light rounded h-100 p-4">
                        <h6 class="mb-4" style="font-size: x-large;
                            ...
                            text-align: center;">List of All Users
                        </h6>
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    <tr v-for='(i, idx) in user' :key="idx">
                                        ...
                                        <td>{{ i.dob }}</td>
                                        <td>{{ i.health_id }}</td>
                                        <td><a href="">curepe.in/file</a></td>
                                        <td><a href="">curepe.in/file</a></td>
                                        <td>Done</td>
                                        <td>
                                            <router-link to="/memberlist/">Added member list</router-link>
                                        </td>
...
export default {
    ...
    method: {
        async isOTPVarified() {
            let token = localStorage.getItem('access_token')
            await axios.get(
                'https://api-cure-pe.synchsoft.in/api/v1/added_member_listzz', {
                headers: {
                    'Authorization': token,
                }
            }
                ...
                // localStorage.setItem('access_token',token)
                if (res) {
                    this.user = res.data.data_relatives
                }
            }).catch(error => {
                console.log(error)
            })
